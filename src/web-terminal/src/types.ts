...
export type TerminalInput =
  ...
  | ({ type: "pointer"; button: PointerButton; point: Readonly<TerminalPoint> } & InputModifiers)
  | ({ type: "wheel"; deltaX: number; deltaY: number; deltaMode: number;
       point: Readonly<TerminalPoint> | null } & InputModifiers)
...
export type InputActionHandler =
  (context: TerminalInputContext, args: unknown, input: Readonly<TerminalInput> | undefined) => unknown;
...
export type InputInterceptor =
  (input: Readonly<TerminalInput>, context: TerminalInputContext) => InputRouteValue | InputDecision | undefined;
...
export interface RunTerminalAction {
  ...
  <Name extends string>(action: Name extends TerminalActionName ? never : Name,
