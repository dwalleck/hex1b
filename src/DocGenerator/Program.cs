...
public partial class Program
{
    ...
    private static string FindProjectRoot()
    {
        ...
        while (dir != null)
        {
            if (File.Exists(Path.Combine(dir, "apphost.cs")))
                return dir;
            dir = Directory.GetParent(dir)?.FullName;
        }
        throw new InvalidOperationException("Could not find project root (looking for apphost.cs)");
    }
}
...
/// </summary>
public partial class YamlToMarkdownConverter
{
    private readonly string _yamlDir;
    private readonly string _outputDir;
    private readonly Dictionary<string, ApiItem> _items = new();

    public YamlToMarkdownConverter(string yamlDir, string outputDir)
    {
        _yamlDir = yamlDir;
...
public class ApiItem
{
    ...
    public List<string> Inheritance { get; } = new();
    public List<string> Implements { get; } = new();
    public List<ParameterItem> Parameters { get; } = new();
}
